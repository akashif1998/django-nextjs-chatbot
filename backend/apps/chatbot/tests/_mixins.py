...
class ChatbotTestMixin:
    ...
    # ------------------------------------------------------------------
    # WebSocket auth (ChatConsumer tests)
    # ------------------------------------------------------------------

    @staticmethod
    def create_ws_access_token(user) -> str:
        """Create a JWT access token string for ``?token=`` WS auth in tests.

        Pure local crypto (rest_framework_simplejwt) — no external call.
        """
