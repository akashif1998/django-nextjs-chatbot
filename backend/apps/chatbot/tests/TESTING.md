...
python manage.py test chatbot.tests.test_streaming --settings=config.settings.test -v 2
...
| `test_streaming.py` | 19 | WebSocket streaming regressions: tool-output leak in `astream()`, non-blocking `SummarizationMiddleware.abefore_model`, `ChatConsumer` orchestrator reuse + overlapping-stream guard |
| **Total** | **~220** | |
...
| `create_ws_access_token(user)` | JWT string for `?token=` WebSocket auth (local crypto, no external call) |
...
## Streaming Fixes Test Coverage (`test_streaming.py`)

This file is the suite's first use of **async test methods**
(`async def test_...`) — supported natively by Django's `TestCase` since
Django 4.1, no `pytest-asyncio` needed. It also introduces
`channels.testing.WebsocketCommunicator` for consumer-level tests. Both
patterns are documented here for the next person adding one:

- An `async def test_...` method on a `TestCase` subclass just works —
  Django runs it via `asyncio.run()` and handles ORM/transaction
  thread-safety automatically.
- Sync code that touches the ORM (e.g. constructing `ChatAgentOrchestrator`,
  whose `__init__` does tool-loading queries) must still be wrapped in
  `channels.db.database_sync_to_async(...)` when called from an async test
  — exactly like production call sites (`AgentService.astream`,
  `ChatConsumer.connect`) already do. Skipping this raises
  `SynchronousOnlyOperation`.
- `WebsocketCommunicator` tests need a real `CHANNEL_LAYERS` backend
  (`connect()` calls `channel_layer.group_add`) — `config.settings.test`
  inherits the Redis-backed layer from `development.py`, so Redis must be
  reachable (it's local infra, not covered by the "no external calls"
  rule, which targets paid LLM/OCR/payment APIs).

| Component | Tests | Scenarios |
|-----------|------:|-----------|
| `_extract_text_delta` | 6 | str passthrough, None, content-block lists (text/mixed/plain-string), empty list |
| `ChatAgentOrchestrator.astream()` | 4 | ToolMessage content excluded from the token stream, AI chunks yielded in order, content-block chunks normalised, empty tool-call chunks skipped |
| `SummarizationMiddleware` async path | 4 | `abefore_model` overridden (so `create_agent` won't fall back to the blocking sync hook), disabled/below-threshold skip, above-threshold uses `acompress_messages` and never touches the sync `compress_messages` |
| `SummarizationService` async methods | 4 | `agenerate_summary` no-op + `ainvoke`-only (never `invoke`), `acompress_messages` no-op below `keep_recent`, compression preserves system prefix + recent messages |
| `ChatConsumer` orchestrator reuse | 2 | One orchestrator built per connection and reused across messages, graceful close (code 4005) if orchestrator construction fails |
| `ChatConsumer` streaming guard | 1 | A second message while one is still streaming is rejected with a clear error instead of running concurrently |

### Running just the streaming tests
