...
class SummarizationMiddleware(AgentMiddleware):
    ...
    def before_model(self, state: Dict[str, Any]) -> Dict[str, Any]:
        """
        Compress messages before sending to the LLM (sync path).

        This is the middleware equivalent of the old ``pre_model_hook``.
        Used when the agent runs via ``.invoke()``/``.stream()`` from a
        synchronous context (Django views, Celery tasks). WebSocket
        consumers use ``astream()``, which calls ``abefore_model`` below
        instead — see that method's docstring for why this sync version
        must never be reached from async code.
        """
    ...
    async def abefore_model(
        ...
        """
        Compress messages before sending to the LLM (async path).

        Without this method, LangGraph's ``RunnableCallable.ainvoke()``
        falls back to calling the sync ``before_model`` directly inside the
        running event loop (it has no thread-offload fallback for graph
        middleware nodes). ``before_model`` -> ``compress_messages`` ->
        ``ChatOpenAI.invoke()`` is a blocking network call, so every
        WebSocket connection on the process would freeze for the duration
        of that call — confirmed live: once a session's history passes the
        threshold, plain HTTP requests to the server stalled for 1.4-1.7s
        on every subsequent turn. Defining this async hook makes
        ``create_agent`` route the async graph through ``acompress_messages``
        (``model.ainvoke()``) instead, so the event loop is never blocked.
        """
        if not self._config.get("enabled", True):
            return state
        ...
        compressed = await SummarizationService.acompress_messages(
            messages=messages,
            keep_recent=self._config.get("keep_recent", 10),
            model_name=self._config.get("model_name", "gpt-4o-mini"),
            max_summary_tokens=self._config.get("max_summary_tokens", 128),
        ...
        logger.info(
            "SummarizationMiddleware (async): compressed %d → %d messages",
            len(messages),
            len(compressed),
        )
...
def _extract_text_delta(content: Any) -> str:
    """Normalise an ``AIMessageChunk.content`` payload into plain text.

    Most chat completions yield a plain ``str``. LangChain v1's standard
    content-block format (used by reasoning/multimodal models) instead
    yields a ``list`` of block dicts (e.g. ``[{"type": "text", "text": "..."}]``).
    Concatenating a list onto ``accumulated_response`` would raise
    ``TypeError``, so both shapes are handled here.
    """
...
class ChatAgentOrchestrator:
    ...
    async def astream(self, user_message: str):
        ...
        async for event in self.agent.astream(
            ...
            # IMPORTANT: only AIMessageChunk carries the assistant's actual
            # reply tokens. When the agent calls a tool, this same stream
            # also emits the ToolMessage produced by the tool node — its
            # .content is the tool's raw return value (e.g. "2541594" for
            # the calculator), NOT part of the assistant's answer. Yielding
            # it unconditionally corrupts the response by prepending the
            # raw tool output ahead of the real text.
            if not isinstance(chunk, AIMessageChunk):
                continue

            content = _extract_text_delta(getattr(chunk, "content", None))
            if content:
