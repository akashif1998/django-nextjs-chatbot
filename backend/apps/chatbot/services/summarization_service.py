...
class SummarizationService:
    ...
    @staticmethod
    def _format_conversation_text(messages: List[BaseMessage]) -> str:
    ...
    def generate_summary(
        ...
        """
        Generate a text summary from a list of messages (sync — blocking).

        Only call this from a synchronous context (Django views, Celery
        tasks, management commands). From async code (WebSocket consumers)
        use ``agenerate_summary`` instead — this uses ``model.invoke()``,
        which would block the asyncio event loop if awaited-around.
        ...
        conversation_text = SummarizationService._format_conversation_text(messages)
    ...
    async def agenerate_summary(
        ...
        """
        Async counterpart to ``generate_summary`` — uses ``model.ainvoke()``.

        Use this from async LangGraph hooks (e.g.
        ``SummarizationMiddleware.abefore_model``) so the summarization LLM
        call never blocks the event loop that's also serving other
    ...
    def _split_for_compression(
        ...
        """
        Split messages into (system_prefix, conversation_messages, recent_messages)
        ready for summarization, or ``None`` if there's nothing to compress
        (too few messages, or only a leading SystemMessage with no body).
        """
        ...
        return system_prefix, conversation_messages, recent_messages
    ...
    def compress_messages(
        ...
        (sync — blocking). Only call from a synchronous context; from async
        code use ``acompress_messages`` instead.
        ...
        """
        split = SummarizationService._split_for_compression(messages, keep_recent)
        if split is None:
            return messages
        system_prefix, conversation_messages, recent_messages = split
    ...
    async def acompress_messages(
        ...
        """
        Async counterpart to ``compress_messages`` — uses ``agenerate_summary``
        so the summarization LLM call never blocks the event loop.

        Use this from async LangGraph hooks (``SummarizationMiddleware.abefore_model``),
        i.e. the WebSocket/``astream()`` path.
        """
        split = SummarizationService._split_for_compression(messages, keep_recent)
        if split is None:
            return messages
        ...
        logger.info(
            "Summarizing %d old messages async (keeping %d recent, style=%s)",
            len(conversation_messages),
            keep_recent,
            style,
        )

        summary_text = await SummarizationService.agenerate_summary(
            messages=conversation_messages,
            model_name=model_name,
            max_summary_tokens=max_summary_tokens,
            style=style,
        ...
        logger.info(
            "Compressed %d messages → %d (%d tokens saved) [async]",
            len(messages),
            len(compressed),
            SummarizationService.get_token_count(messages)
            - SummarizationService.get_token_count(compressed),
